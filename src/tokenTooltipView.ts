...
export class TokenTooltipView {
	public static currentPanel: TokenTooltipView | undefined;
	...
	public static createOrShow(extensionUri: vscode.Uri, tokenKey: string, tokenResolver: TokenResolver): void {
		...
		if (TokenTooltipView.currentPanel) {
			TokenTooltipView.currentPanel._panel.reveal(column);
			TokenTooltipView.currentPanel.update(tokenKey, tokenResolver);
		} else {
			const panel = vscode.window.createWebviewPanel("tokenTooltip", `Token Tooltip: ${tokenKey}`, column, {
				enableScripts: true,
				retainContextWhenHidden: true,
			});
			TokenTooltipView.currentPanel = new TokenTooltipView(panel, extensionUri, tokenKey, tokenResolver);
		}
	}
	...
	public dispose(): void {
		TokenTooltipView.currentPanel = undefined;
		this._panel.dispose();
